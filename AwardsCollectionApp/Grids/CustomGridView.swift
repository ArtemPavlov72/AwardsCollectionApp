...
struct CustomGridView<Content, T>: View where Content: View { //чтобы сделать универсальную сетку, чтобы она принимала любой тип данных (дженерик), мы определяем тип через Т
    ...
    let content: (T) -> Content //захватывает тип данных и возвращает View
    ...
    var body: some View {
        GeometryReader { geometry in // для работы с размерами ячеек
            let sideLength = geometry.size.width / CGFloat(columns) //определяем размер одной стороны. Берем ширину экрана и делим на количество колонок
            ScrollView {
